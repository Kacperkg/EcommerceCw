...
const LoginForm = ({ onLogin, onBack }: { onLogin: (user: { fullName: string; email: string; password: string }) => void, onBack: () => void }) => {
  ...
  const handleLogin = async () => {
    try {
      ...
      if (userDoc.exists() && userDoc.data().password === password) {
        onLogin(userDoc.data() as { fullName: string; email: string; password: string });
  ...
  return (
    <div>
      ...
      <button onClick={handleLogin} style={{ backgroundColor: "#7A746E", color: "#FFFFFF", padding: "0.5rem 1rem", borderRadius: "0.5rem", marginRight: "0.5rem" }}>
        Login
      </button>
